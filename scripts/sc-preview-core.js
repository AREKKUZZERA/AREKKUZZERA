...
export function buildHtml(template, data) {
  ...
  return replaceAllPlaceholders(template, {
    ...
    REFRESH_LABEL: escapeHtml(firstValue(data?.refreshLabel, data?.refresh_label, "Data refreshes every 30 sec")),
    PERIOD_LABEL: escapeHtml(firstValue(data?.periodLabel, data?.period_label, `${data?.sinceYear ?? 2016} - Present`)),
    STATUS_LABEL: escapeHtml(firstValue(data?.statusLabel, data?.status_label, "Live stats connected")),
